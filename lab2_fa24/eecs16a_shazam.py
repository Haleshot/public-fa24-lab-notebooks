...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""First, let's look at the DFT of our signals.""")
    return
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""As expected, the *pure tones* (the 100 Hz and 400 Hz sine waves) have 2 peaks each, whereas the signal formed by concatenating them has 4 peaks. Now, let's look at the spectrograms of these signals. Run the following code to plot the spectrogram of each signal.""")
    return


@app.cell
def _(plt, signal, x1, x2, x3):
    f1, t1, x1_freqs = signal.spectrogram(x1, fs=1000)
    _f2, _t2, x2_freqs = signal.spectrogram(x2, fs=1000)
    f3, t3, x3_freqs = signal.spectrogram(x3, fs=1000)
    _fig, _axs = plt.subplots(1, 3, figsize=(16, 4), dpi=200)
    _axs[0].pcolormesh(t1, f1, x1_freqs, cmap='gray', shading='auto')
    _axs[0].set_ylabel('Frequency [Hz]')
    _axs[0].set_xlabel('Time [sec]')
    _axs[0].set_title('100 Hz Signal')
    _axs[1].pcolormesh(_t2, _f2, x2_freqs, cmap='gray', shading='auto')
    _axs[1].set_ylabel('Frequency [Hz]')
    _axs[1].set_xlabel('Time [sec]')
    _axs[1].set_title('400 Hz Signal')
    _axs[2].pcolormesh(t3, f3, x3_freqs, cmap='gray', shading='auto')
    _axs[2].set_ylabel('Frequency [Hz]')
    _axs[2].set_xlabel('Time [sec]')
    _axs[2].set_title('100 Hz and 400 Hz Signal')
    plt.gca()
    return f1, f3, t1, t3, x1_freqs, x2_freqs, x3_freqs
...
def _(coldplay_1, compute_spectrogram, fs_1, killers, plt):
    f1_1, t1_1, coldplay_spect = compute_spectrogram(fs_1, coldplay_1, epsilon_db_constant=1e-12)
    _f2, _t2, killers_spect = compute_spectrogram(fs_1, killers, epsilon_db_constant=1e-12)
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""**Q:** In both spectrograms, we see a column of dark blue for the first second or so. Based on our colorbar, it looks like this corresponds to $\approx -300 \text{dB}$, or essentially no signal power. In terms of the songs, why do we have this in our plots?""")
    return


@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""**Q:** At the beginning of the spectrogram for Mr. Brightside (after the column of dark blue), you should see two peaks that extend up toward $20 \text{ kHz}$. What sound in the song is this part of the spectrogram capturing?""")
    return


@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""**Q:** Can you easily tell the two songs' spectrograms apart? Do you think they'd make good building blocks for our audio recognition algorithm?""")
    return
...
def _(coldplay_spect, f1_1, freq_indices, plt, t1_1, time_indices):
    ...
    plt.pcolormesh(t1_1, f1_1, coldplay_spect, zorder=0, cmap='jet', shading='auto')
    plt.ylabel('Frequency [Hz]')
    plt.xlabel('Time [sec]')
...
@app.cell
def _(add_gaussian_noise, coldplay_2, fs_2, get_20_second_segment, ipd):
    ipd.Audio(add_gaussian_noise(get_20_second_segment(fs_2, coldplay_2)).T, rate=fs_2)
    return
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""**Q:** What happens in the frequency domain when we zero out parts of the signal in the time domain?""")
    return
